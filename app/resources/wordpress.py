...
from models.wpAdmin import WpModel
...
class Wordpress(Resource):
    ...
    def get(self, name):
        item = WpModel.find_by_name(name)
        if item:
            return item.json()
    ...
    # @fresh_jwt_required
    def post(self, name):
        if WpModel.find_by_name(name):
            return {'message': "An item with name '{}' already exists.".format(name)}, 400

        data = Item.parser.parse_args()

        item = WpModel(name, **data)
...
class WordpressList(Resource):
    ...
    def get(self):
        ...
        items = [item.json() for item in WpModel.find_all()]
        if user_id:
