...
from models.accessiDb import DbModel
...
class Db(Resource):
    ...
    def get(self, name):
        item = DbModel.find_by_name(name)
        if item:
            return item.json()
    ...
    # @fresh_jwt_required
    def post(self, name):
        if DbModel.find_by_name(name):
            return {'message': "An item with name '{}' already exists.".format(name)}, 400

        data = Item.parser.parse_args()

        item = DbModel(name, **data)
...
class DbList(Resource):
    ...
    def get(self):
        ...
        items = [item.json() for item in DbModel.find_all()]
        if user_id:
