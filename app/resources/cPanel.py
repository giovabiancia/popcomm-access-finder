...
from models.cPanel import CpanelModel


class Cpanel(Resource):
    ...
    def get(self, name):
        item = CpanelModel.find_by_name(name)
        if item:
            return item.json()
    ...
    # @fresh_jwt_required
    def post(self, name):
        if CpanelModel.find_by_name(name):
            return {'message': "An item with name '{}' already exists.".format(name)}, 400

        data = Item.parser.parse_args()

        item = CpanelModel(name, **data)
...
class CpanelList(Resource):
    ...
    def get(self):
        ...
        items = [item.json() for item in CpanelModel.find_all()]
        if user_id:
